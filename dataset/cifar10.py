...
def compute_mean_std(cifar10_dataset):
    ...
    """
    data = cifar10_dataset.data/255
    mean = data.mean(axis=(0,1,2))
    std = data.std(axis=(0,1,2))
    print("mean: {}, std: {}".format(mean, std))
    return mean, std

CIFAR010_TRAIN_MEAN,CIFAR10_TRAIN_STD= compute_mean_std(CIFAR10(root='./data',train=True,download=True,transform=None))  
...
def get_CIFAR10_train_loader(batch_size, rand_fraction=0.0, num_workers=1, shuffle=True):
    """ CIFAR10 training set loader """
    cifar10_train_dataset = CIFAR10WithIdx(root='./data',
                                           train=True,
                                           download=True,
...
def get_CIFAR10_test_loader(batch_size, num_workers=1, shuffle=False):
    """ CIFAR10 test set loader """
    cifar10_test_dataset = CIFAR10WithIdx(root='./data',
                                          train=False,
                                          download=True,
...
def get_CIFAR10_train_valid_split_loader(batch_size, rand_fraction=0.0, num_workers=1, shuffle=True, valid_size=0.1):
    """ CIFAR10 training set loader """
    cifar10_train_dataset = CIFAR10WithIdx(root='./data',
                                           train=True,
                                           download=True,
    ...
    cifar10_valid_loader = torch.utils.data.DataLoader(dataset=cifar10_train_dataset,
                                                       batch_size=batch_size,
                                                       shuffle=False,
                                                       num_workers=num_workers,
                                                       sampler=valid_sampler)
    return cifar10_train_loader, cifar10_valid_loader